...
NOTE: SZA bands are larger than color bands, which leads to the white stripe in the figure below. Perhaps buffering the mask used to patch based on SZA scenes would fix this.
