...
def ozone_scattering(ozone, tozon, sza, vza, toa):
    scale = np.arccos(-1.0) / 180.0  # rad per degree
    eps = 1.55
    # ecmwf ozone from OLCI file (in Kg.m-2) to DOBSON UNITS
    # 1 kg O3 / m2 = 46696.24  DOBSON Unit (DU)
    totadu = 46729.0 * ozone

    amf = 1.0 / np.cos(sza * scale) + 1.0 / np.cos(vza * scale)

    BX = (toa[20, :, :] ** (1.0 - eps)) * (toa[16, :, :] ** eps) / toa[6, :, :]
    ...
    for i in range(21):
        ...
        toa_cor_o3[i, :, :] = (
            toa[i, :, :] * tvoda[i] * np.exp(amf * tozon[i] * totadu / 404.59)
        )
...
def view_geometry(vaa, saa, sza, vza, aot, height):
    ...
    raa = 180.0 - (vaa - saa)
    as1 = np.sin(sza * np.pi / 180.0)
    as2 = np.sin(vza * np.pi / 180.0)

    am1 = np.cos(sza * np.pi / 180.0)
    am2 = np.cos(vza * np.pi / 180.0)

    ak1 = 3.0 * (1.0 + 2.0 * am1) / 7.0
    ak2 = 3.0 * (1.0 + 2.0 * am2) / 7.0

    cofi = np.cos(raa * np.pi / 180.0)
    amf = 1.0 / am1 + 1.0 / am2
...
def aerosol_properties(aot, height, co):
    ...
    ad = height / 7400.0
    ak = height * 0 + 1
    ak[ad > 1.0e-6] = np.exp(-ad[ad > 1.0e-6])
...
def prepare_coef(tau, g, p, am1, am2, amf, gaer, taumol, tauaer):
    ...
    # SOBOLEV
    oskar = 4.0 + 3.0 * (1.0 - g) * tau
    b1 = 1.0 + 1.5 * am1 + (1.0 - 1.5 * am1) * np.exp(-tau / am1)
    b2 = 1.0 + 1.5 * am2 + (1.0 - 1.5 * am2) * np.exp(-tau / am2)
    ...
    sssss = (wa1 - wa2) / (1.0 + bex) + wa2

    for i in range(21):

        astra[i, :, :] = (1.0 - np.exp(-tau[i, :, :] * amf)) / (am1 + am2) / 4.0
        rms[i, :, :] = (
            1.0
            - b1[i, :, :] * b2[i, :, :] / oskar[i, :, :]
            + (3.0 * (1.0 + g[i, :, :]) * am1 * am2 - 2.0 * (am1 + am2))
            * astra[i, :, :]
        )
        ...
        t1[i, :, :] = np.exp(
            -(1.0 - g[i, :, :]) * tau[i, :, :] / am1 / 2.0 / sssss[i, :, :]
        )
        t2[i, :, :] = np.exp(
            -(1.0 - g[i, :, :]) * tau[i, :, :] / am2 / 2.0 / sssss[i, :, :]
        )
    ...
    # SALBED
    # ratm = salbed(tau, g)
    a_s = (0.18016, -0.18229, 0.15535, -0.14223)
    bs = (0.58331, -0.50662, -0.09012, 0.0207)
    ...
    ratm = tau * (
        a_cst * np.exp(-tau / al_cst) + b_cst * np.exp(-tau / bet_cst) + c_cst
    )
...
def snow_impurities(alb_sph, bal):
    ...
    ind_nonan = np.logical_and(
        np.logical_not(np.isnan(alb_sph[0, :, :])),
    ...
    alfa = 4.0 * np.pi * 0.47 / w[0]  # bulk soot absorption coefficient at 1000nm
    ...
    ntype[bm >= 10.0] = 4  # type is other or mixture
...
def plane_albedo_sw_approx(D, am1):
    anka = 0.7389 - 0.1783 * am1 + 0.0484 * am1 ** 2.0
    banka = 0.0853 + 0.0414 * am1 - 0.0127 * am1 ** 2.0
    canka = 0.1384 + 0.0762 * am1 - 0.0268 * am1 ** 2.0
    diam1 = 187.89 - 69.2636 * am1 + 40.4821 * am1 ** 2.0
    diam2 = 2687.25 - 405.09 * am1 + 94.5 * am1 ** 2.0
