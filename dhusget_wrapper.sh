...
print_usage() {
	...
	echo "  -pl path to product list previously downloaded from esa servers"
...
if [[ -z ${footprint:-} ]]; then
	footprint=Greenland
	log_warn "Footprint not set. Setting to Greenland"
else
...
# case where no product list is available: fetching list with dhusget
if [[ -z ${path_product_list:-} ]]; then
