...
def get_effective_angle(variable):
    ...
    mu = np.cos(angle_rad) * np.cos(slope_rad) + np.sin(angle_rad) * np.sin(
        slope_rad
    ...
    try:
        ...
        profile = {
            k: v for k, v in profile.items() if k not in ["blockxsize", "blockysize"]
        }
        with rasterio.open(output_filename, "w", **profile) as dst:
            dst.write(angle_eff, 1)
    ...
    slope_flag_filename = (
        args.inpath + "slope_flag_" + str(slope_thres) + "_degrees.tif"
    )
...
def get_ITOAR(slope, aspect):
    ...
    def compute_ITOAR(toar):
        ...
        mu0_ov = mu0 * np.cos(np.deg2rad(slope)) + np.sin(np.deg2rad(sza)) * np.sin(
            np.deg2rad(slope)
        ) * np.cos(np.deg2rad(saa) - np.deg2rad(aspect))
