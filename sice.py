...
t1, t2, ratm, r, astra, rms = sl.prepare_coef(
...
alb_sph = np.exp(
    -np.sqrt(1000.0 * 4.0 * np.pi * mult_channel(bai / w, np.tile(al, (21, 1, 1))))
)
...
am11 = np.sqrt(1.0 - am1[ind_very_dark] ** 2.0)
am12 = np.sqrt(1.0 - am2[ind_very_dark] ** 2.0)

tz = (
    np.arccos(
        -am1[ind_very_dark] * am2[ind_very_dark]
        + am11 * am12 * np.cos(raa[ind_very_dark] * 3.14159 / 180.0)
    )
    * 180.0
    / np.pi
)
...
rclean = (
    1.247
    + 1.186 * (am1[ind_very_dark] + am2[ind_very_dark])
    + 5.157 * am1[ind_very_dark] * am2[ind_very_dark]
    + pz
)

rclean = rclean / 4.0 / (am1[ind_very_dark] + am2[ind_very_dark])
...
if np.any(ind_pol):
    ...
    def solver_wrapper(toa_cor_o3, tau, t1, t2, r0, ak1, ak2, ratm, r):
        ...
        return sl.zbrent(func_solv, 0.1, 1, 100, 1.0e-6)
    ...
    for i_channel in range(21):
        alb_sph[i_channel, ind_clear_pol] = np.exp(
            -np.sqrt(
                4.0 * 1000.0 * al[ind_clear_pol] * np.pi * bai[i_channel] / w[i_channel]
            )
        )
    ...
    for i_channel in range(17, 21):
        alb_sph[i_channel, ind_ok] = np.exp(
            -np.sqrt(4.0 * 1000.0 * al[ind_ok] * np.pi * bai[i_channel] / w[i_channel])
        )
...
for i in np.append(np.arange(11), np.arange(15, 21)):
    ...
    WriteOutput(
        rp[i, :, :], "albedo_spectral_planar_" + str(i + 1).zfill(2), InputFolder
    )
    WriteOutput(refl[i, :, :], "rBRR_" + str(i + 1).zfill(2), InputFolder)

print(
    "End SICE.py %s --- %s CPU seconds ---"
