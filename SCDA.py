...
def radiometric_calibration(R16, scene, inpath=args.inpath):
    ...
    with rasterio.open(
        inpath + os.sep + scene + os.sep + "r_TOA_S5_rc.tif", "w", **profile_R16
    ) as dst:
        dst.write(R16_rc, 1)


def SCDA_v20(
    R550, R16, BT37, BT11, BT12, profile, scene, inpath=args.inpath, SICE_toolchain=True
):
    ...
    with rasterio.open(
        inpath + os.sep + scene + os.sep + "NDSI.tif", "w", **profile
    ) as dst:
        dst.write(NDSI, 1)
    ...
    with rasterio.open(
        inpath + os.sep + scene + os.sep + "SCDA_v20.tif",
        "w",
...
for i, scene in enumerate(scenes):
    ...
    profile = rasterio.open(
        args.inpath + os.sep + scene + os.sep + "r_TOA_S1.tif"
    ).profile
    ...
    R16 = rasterio.open(args.inpath + os.sep + scene + os.sep + "r_TOA_S5_rc.tif").read(
        1
    )
