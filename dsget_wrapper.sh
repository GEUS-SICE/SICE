...
# of the Data Hub Service (Ds) - previsouly Data Hub Service (DHuS)                         #
#-------------------------------------------------------------------------------------------#
# CHANGE LOG
# v0.3.1:
#	- usage switch fixed
...
#CHANGE LOG
#v0.3.9
#		- translated Copernicus scihub OData (Open Data Protocol) to
#                 Copernicus dataspace OData
#                 (see https://scihub.copernicus.eu/news/News01249)  
#
#-------------------------------------------------------------------------------------------	#
...
function print_usage {
	...
	echo " "
	echo "  Dsget $VERSION - The non interactive Sentinels product retriever from the Sentinels Data Hubs"
	echo " "
	...
	echo " "
	echo "   -d <Ds URL>		: specify the URL of the Data Hub Service;"
	...
	echo " "
	echo "   -L <lock folder>		: by default only one instance of dsget can be executed at a time. This is ensured by the creation"
	echo "   				  of a temporary lock folder $HOME/dsget_tmp/lock which is removed a the end of each run."
	echo "   				  For running more than one dsget instance at a time is sufficient to assign different lock folders"
	echo "   				  using the -L option (e.g. '-L foldername') to each dsget instance;"
	echo " "
	...
	echo " "
	echo " "
	echo "   'wget' is necessary to run the dsget"
	echo " "
	exit -1
}
...
function download_product {
	...
	echo "Downloading product ${3} from link ${DS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";	
# ---added control on S5P products
        prod5P=${3}
		mission5P="S5P"
		if echo $prod5P  | grep -q "$mission5P"; then
	${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=./logs/log.${3}.log -O $output_folder/${3}".nc" "${DS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	else
	
        ${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=./logs/log.${3}.log -O $output_folder/${3}".zip" "${DS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	fi
	test=$?;
# --- check HTTP response status in case of offline products download	
	...
	if [ ! -z "$http_response" ]; then
		...
		if [ $test -eq 0 ]; then 
			...
			remoteMD5=$( ${WC} -qO- ${AUTH} ${TRIES} -c "${DS_DEST}/odata/v1/Products('\''"$1"'\'')/Checksum/Value/$value" | awk -F">" '\''{print $3}'\'' | awk -F"<" '\''{print $1}'\'');
			if echo $prod5P  | grep -q "$mission5P"; then 
...
export DS_DEST="https://catalogue.dataspace.copernicus.eu/odata/v1"
export USERNAME=""
export PASSWORD=""
...
while getopts ":d:u:p:l:P:q:C:m:i:t:s:e:S:E:f:c:T:o:V:h:F:R:D:r:O:N:L:n:w:W:" opt; do
	case $opt in
	d)
		export DS_DEST="$OPTARG"
		export DS_DEST_IS_SET='OK'
		;;
	u)
...
export SENSING_SUBQUERY="ContentDate/Start gt $SENSING_TIME_FROM and ContentDate/Start lt $SENSING_TIME_TO  "
...
if [ -z $DS_DEST_IS_SET ]; then
	echo "'-d option' not specified. "
	echo "Default Data Space Service URL is: "
else
	echo "Specified Data Space Service URL is:"
fi
echo $DS_DEST
...
if [ ! -z $LOCK_FILE_IS_SET ]; then
	echo "'-L option' is set to $lock_file. This instance of dsget can be executed in parallel to other instances. "
	echo ""
fi
...
if [[ ! -z $x1 ]]; then
	...
	export GEO_SUBQUERY=$(
		...
		printf "( OData.CSC.Intersects(area=geography'SRID=4326;POLYGON ((%.13f %.13f,%.13f %.13f,%.13f %.13f,%.13f %.13f,%.13f %.13f )))\")" $x1 $y1 $x2 $y1 $x2 $y2 $x1 $y2 $x1 $y1
	)
...
START=$((PAGE - 1))
START=$((START * ROWS))
export QUERY_STATEMENT="${DS_DEST}/Products?\$filter="${QUERY_STATEMENT}""
