...
parser.add_argument(
    "file", help="File used to create boundary: csv and geojson extensions implemented"
)
    ...
    if extension == "geojson":
        for i in range(0, np.shape(coordinates)[0]):
            if i == np.shape(coordinates)[0] - 1:
                ...
                boundary = (
                    boundary + str(coordinates[0][0]) + " " + str(coordinates[0][1])
                )
            else:
                boundary = (
                    boundary
    ...
    if extension == "csv":
        for i in range(0, coordinates.shape[0]):
            if i == coordinates.shape[0] - 1:
                boundary = (
                    boundary
                    + str(coordinates.iloc[i][0])
                    + " "
                    + str(coordinates.iloc[i][1])
                    + ","
                )
                boundary = (
                    boundary
                    + str(coordinates.iloc[0][0])
                    + " "
                    + str(coordinates.iloc[0][1])
                )
            else:
                boundary = (
                    boundary
